...
def sh(cmd, t=120):
    r = subprocess.run(["bash", "-c", cmd], capture_output=True, text=True, timeout=t, cwd=ROOT)
...
while True:
    ...
    log(f"LLM ({len(p)}c)...")
    t0 = time.time()
    resp = client.chat.completions.create(model="Qwen3.6-27B-Q4_K_M.gguf", messages=[{"role": "user", "content": p}], max_tokens=200, temperature=0.1)
